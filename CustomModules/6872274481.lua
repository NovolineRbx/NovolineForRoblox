...
local function vapeGithubRequest(scripturl)
	if not isfile("vape/"..scripturl) then
		local suc, res = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/NovolineRbx/NovolineForRoblox/"..readfile("vape/commithash.txt").."/"..scripturl, true) end)
		assert(suc, res)
...
--[[run(function() -- mm i love broken modules!
	local SetEmote = {}
	local SetEmoteList = {Value = ''}
	local oldemote
	local emo2 = {}
	SetEmote = GuiLibrary.ObjectsThatCanBeSaved.NovolineWindow.Api.CreateOptionsButton({
	Name = 'SetEmote',
...
run(function() -- i dont know why bedwars hasnt patched it but they havent (ive had this for a month i believe by now)
	...
	MelodyExploit = GuiLibrary.ObjectsThatCanBeSaved.NovolineWindow.Api.CreateOptionsButton({ -- how does this work? idk honestly
		Name = "MelodyExploit",
		Function = function(callback)
			if callback then
...
run(function()
	...
	JellyfishExploit = GuiLibrary.ObjectsThatCanBeSaved.NovolineWindow.Api.CreateOptionsButton({
		Name = "JellyfishExploit",
		Function = function(callback)
			if callback then
		warningNotification("JellyFishExploit", "Requires marina kit", 5)																																																																																																																																																																																																																																													-- thank you whoever gave me this
                task.spawn(function()
...
run(function()
    local CustomModule = GuiLibrary.ObjectsThatCanBeSaved.NovolineWindow.Api.CreateOptionsButton({
        Name = "HealthbarMods",
        Function = function(callback)
            if callback then
                spawn(function()
                    pcall(function()
                        ...
                        local speed = 1
                        local redStart, greenStart, blueStart = 0, 0, 255
                        local redEnd, greenEnd, blueEnd = 0, 0, 255
                        local healthbar

                        repeat
                            if not healthbar or not healthbar.Parent then
                                local hotbar = PlayerGui:WaitForChild("hotbar")
                                local hotbar1 = hotbar:WaitForChild("1")
                                local hotbarHealthbarContainer = hotbar1:WaitForChild("HotbarHealthbarContainer")
                                local healthbarProgressWrapper = hotbarHealthbarContainer:WaitForChild("HealthbarProgressWrapper")
...
run(function()
	...
	HannahExploit = GuiLibrary.ObjectsThatCanBeSaved.NovolineWindow.Api.CreateOptionsButton({
		Name = "HannahExploit",
		Function = function(callback)
			if callback then
...
run(function()
    ...
    AutoUpgradeStats = GuiLibrary.ObjectsThatCanBeSaved.NovolineWindow.Api.CreateOptionsButton({
