...
local function vapeGithubRequest(scripturl)
	if not isfile("vape/"..scripturl) then
		local suc, res = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/NovolineRbxx/NovolineForRoblox/"..readfile("vape/commithash.txt").."/"..scripturl, true) end)
		assert(suc, res)
...
run(function()
	...
	function whitelist:playeradded(v, joined)
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				...
				GuiLibrary.SelfDestruct = function() warningNotification('Novoline', 'No escaping the private members :)', 10) end
				if joined then task.wait(10) end
				if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
					local oldchannel = textChatService.ChatInputBarConfiguration.TargetTextChannel
					local newchannel = cloneref(game:GetService('RobloxReplicatedStorage')).ExperienceChat.WhisperChat:InvokeServer(v.UserId)
					if newchannel then newchannel:SendAsync('helloimusingnovoline') end
					textChatService.ChatInputBarConfiguration.TargetTextChannel = oldchannel
				elseif replicatedStorage:FindFirstChild('DefaultChatSystemChatEvents') then
					replicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer('/w '..v.Name..' helloimusingnovoline', 'All')
				end
			end
		end
	end
	...
	function whitelist:checkmessage(msg, plr)
		...
		if plr == lplr and msg == 'helloimusingnovoline' then return true end
		if self.localprio > 0 and self.said[plr.Name] == nil and msg == 'helloimusingnovoline' and plr ~= lplr then
			self.said[plr.Name] = true
			notif('Novoline', plr.Name..' is using Novoline!', 60)
			self.customtags[plr.Name] = {{text = 'NOVOLINE USER', color = Color3.new(1, 1, 0)}}
		...
		if exp then
			...
			if bubblechat then
				table.insert(vapeConnections, bubblechat.DescendantAdded:Connect(function(newbubble)
					if newbubble:IsA('TextLabel') and newbubble.Text:find('helloimusingnovoline') then
	...
	function whitelist:check(first)
		local whitelistloaded, err = pcall(function()
			local _, subbed = pcall(function() return game:HttpGet('https://github.com/NovolineRbxx/whitelists') end)
			local commit = subbed:find('spoofed_commit_check')
			commit = commit and subbed:sub(commit + 21, commit + 60) or nil
			commit = commit and #commit == 40 and commit or 'main'
			whitelist.textdata = game:HttpGet('https://raw.githubusercontent.com/NovolineRbxx/whitelists/'..commit..'/PlayerWhitelist.json', true)
		end)
			...
			if whitelist.textdata ~= whitelist.olddata then
				if whitelist.data.Announcement.expiretime > os.time() then
					...
					if table.find(targets, tostring(lplr.UserId)) then
						...
						hint.Text = 'NOVOLINE ANNOUNCEMENT: '..whitelist.data.Announcement.text
					...
					task.spawn(function()
						repeat
							if ChatSpammer.Enabled then
								pcall(function()
									textChatService.ChatInputBarConfiguration.TargetTextChannel:SendAsync((#ChatSpammerMessages.ObjectList > 0 and ChatSpammerMessages.ObjectList[math.random(1, #ChatSpammerMessages.ObjectList)] or "novoline on top"))
								end)
							end
							if waitnum ~= 0 then
								task.wait(waitnum)
